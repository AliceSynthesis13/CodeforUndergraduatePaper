function[H] = Real_Hamiltonian(Nx,Ny,type,t,lambda_SO,mu,Delta,N_begin,N_add)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 到了塑形环节了
% 由于拓扑性质只与边界交界有关，这里我们只用 type 来研究，其他类型的没时间编程了
if type == 1
    % AB-C型的，加上一层AB-C了，这个时候要先去掉C，再去掉多出那层AB的部分AB原子
    % 删除C原子
    H(posim(1,Ny,3,type,Nx)+1:posim(Nx,Ny,3,type,Nx)+4,:) = [];
    H(:,posim(1,Ny,3,type,Nx)+1:posim(Nx,Ny,3,type,Nx)+4) = [];
    % 删除AB原子
    H(posim(N_begin+N_add-1,Ny,2,type,Nx)+1:posim(Nx,Ny,1,type,Nx)+4,:) = [];
    H(:,posim(N_begin+N_add-1,Ny,2,type,Nx)+1:posim(Nx,Ny,1,type,Nx)+4) = [];
    H(posim(1,Ny,1,type,Nx)+1:posim(N_begin-1,Ny,2,type,Nx)+4,:) = [];
    H(:,posim(1,Ny,1,type,Nx)+1:posim(N_begin-1,Ny,2,type,Nx)+4) = [];
elseif type == 2
    % C-AB型的，加上一层C-AB了，这个时候要先去掉AB，再去掉多出那层C的部分C原子
    % 删除AB原子
    H(posim(1,Ny,1,type,Nx)+1:posim(Nx,Ny,1,type,Nx)+4,:) = [];
    H(:,posim(1,Ny,1,type,Nx)+1:posim(Nx,Ny,1,type,Nx)+4) = [];
    % 删除C原子
    H(posim(N_begin+N_add,Ny,3,type,Nx)+1:posim(Nx,Ny,3,type,Nx)+4,:) = [];
    H(:,posim(N_begin+N_add,Ny,3,type,Nx)+1:posim(Nx,Ny,3,type,Nx)+4) = [];
    H(posim(1,Ny,3,type,Nx)+1:posim(N_begin-1,Ny,3,type,Nx)+4,:) = [];
    H(:,posim(1,Ny,3,type,Nx)+1:posim(N_begin-1,Ny,3,type,Nx)+4) = [];
elseif type == 3
    % AB-AB型的，加上一层C-AB了，这个时候要先去掉AB，再去掉多出那层C的部分C原子
    % 删除AB原子
    H(posim(1,Ny,1,type,Nx)+1:posim(Nx,Ny,1,type,Nx)+4,:) = [];
    H(:,posim(1,Ny,1,type,Nx)+1:posim(Nx,Ny,1,type,Nx)+4) = [];
    % 删除C原子
    ...
    H(:,posim(1,Ny,3,type,Nx)+1:posim(N_begin-1,Ny,3,type,Nx)+4) = [];
elseif type == 4
    % C-C型的，加上一层AB-C了，这个时候要先去掉C，再去掉多出来那层AB的部分AB原子
    % 删除C原子
    H(posim(1,Ny,3,type,Nx)+1:posim(Nx,Ny,3,type,Nx)+4,:) = [];
    H(:,posim(1,Ny,3,type,Nx)+1:posim(Nx,Ny,3,type,Nx)+4) = [];
    % 删除AB原子
